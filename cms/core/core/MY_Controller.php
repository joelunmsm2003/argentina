...
class MY_Controller extends CI_Controller {
    ...
    function _generar_data( $vista ){
        ...
        switch( $vista){
            ...
            case 'cabecera' : {
                ...
                /// URL de retorno ///
                // Usamos la URL definida en el controlador
    ...
    function getOrders(){
        ...
        // (manteniendo el orden de los nuevos criterios)
        // y además me aseguro que los criterios nuevos estén primeros
        if(is_array($orders)){
            $o = array();
            ...
            switch($values['type']){
                    ...
                    if(trim($this->input->post($key)) != ''){
                        if(isset($values['validate'])){
                            if($this->input->post($key) == $this->input->post($values['validate'])){
                                ...
                                $this->errorHeader('Las contraseñas no coinciden.');
                            }
                        }
                    }else{
                        if(! $update_mode){
                            $this->errorHeader('La contraseña es obligatoria.');
                        }
                    }
                    break;
                ...
                case 'form_dropdown_nueva_opcion':
                    ...
                    if($nueva_opcion = $this->input->post($key.'_nueva_opcion')){
                        ...
                        try{
                            $id_nueva_opcion = $this->{$values['opciones']['modelo']}->save_item($o, false, false);
                        }catch (Exception $e){
                ...
                case 'child_relation_list':
                    ...
                    $itemsCRL = (isset($_POST['itemsCRL']) AND $_POST['itemsCRL']) ? $_POST['itemsCRL'] : array();
...
/* Campo Child relation popup list: Actualizar relación */
     function actualizarRelacionCRPL(){
        ...
        $fk_hermano = $this->uri->segment(3, $this->input->get('fk_hermano'));
        $id_hermano = $this->uri->segment(4, $this->input->get('id_hermano'));
        $fk_propia  = $this->uri->segment(5, $this->input->get('fk_propia'));
        $id_propia  = $this->uri->segment(6, $this->input->get('id_propia'));
        if(! $fk_propia OR !$fk_hermano){
            $mensaje = 'Campo CRPL: Falta un parámetro.';
        ...
        if($relaciones){
            foreach($relaciones as $rel){
                if(! $this->$modelo->delete_item($rel->id)){
                    $mensaje = 'Campo CRPL: No se pudo actualizar la base de datos.';
                    log_message('error',$mensaje);
                    echo $mensaje;
                    return false;
                }
