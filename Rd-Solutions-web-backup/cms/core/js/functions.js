...
function eliminarAjax(id){
	if(confirm("¿Está seguro que desea eliminar este elemento?")){
		...
		$.ajax({
			...
			beforeSend: function() {
				$('.mensaje-feedback')
					.html('<img src="'+DOCUMENT_ROOT+'core/images/ajax-loader.gif">&nbsp;&nbsp;Procesando...')
					.addClass('info').fadeIn();
				$("html, body").animate({ scrollTop: 0 }, "slow");
			},
			success: function(){
				// Quito el elemento del listado 
				$('[data-id='+id+']').closest('tr').animate({'opacity': 0}, 700, function(){ $(this).remove(); });

				$('.mensaje-feedback')
					.html('Se eliminó el elemento seleccionado.')
					.removeClass('info')
			...
			if(eliminar){
				...
				$.each($('.js-AEL-item'), function(i, item){
					if( $(item).attr('checked') ){
						...
						$(item).closest('tr').animate({'opacity': 0}, 700, function(){ $(this).remove(); });
					}else{
				...
				if(! quedan_elementos){
					$('.js-AEL-contenedor tbody').html('<tr><td>No quedan elementos.</td></tr>');
				}
			}
		}else{
